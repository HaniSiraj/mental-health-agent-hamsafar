...
const moodHistoryPanel = document.getElementById('mood-history-panel');
const closeMoodPanel = document.getElementById('close-mood-panel');
const toggleMoodLogForm = document.getElementById('toggle-mood-log-form');
const moodLogForm = document.getElementById('mood-log-form');
const moodLogSubmit = document.getElementById('mood-log-submit');
const moodScoreInput = document.getElementById('mood-score-input');
const moodNoteInput = document.getElementById('mood-note-input');
const moodSummaryText = document.getElementById('mood-summary-text');

// Toggle panel visibility and load data
if (moodHistoryBtn) {
    moodHistoryBtn.addEventListener('click', async () => {
        moodHistoryPanel.classList.toggle('hidden');
        if (!moodHistoryPanel.classList.contains('hidden')) {
            await loadAndRenderMoodChart();
        }
    });
}
...
if (closeMoodPanel) {
    closeMoodPanel.addEventListener('click', () => {
        moodHistoryPanel.classList.add('hidden');
    });
}

if (toggleMoodLogForm) {
    toggleMoodLogForm.addEventListener('click', () => {
        moodLogForm.style.display = moodLogForm.style.display === 'none' ? 'block' : 'none';
    });
}

if (moodLogSubmit) {
    moodLogSubmit.addEventListener('click', async () => {
        const score = parseInt(moodScoreInput.value);
        const note = moodNoteInput.value.trim();
        if (!score || score < 1 || score > 10) {
            addMessage('assistant', 'Please enter a mood score between 1 and 10. 💙');
            return;
        }
        try {
            const resp = await fetch(`${API_BASE}/api/log_mood/${sessionId}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (resp.ok) {
                ...
                moodNoteInput.value = '';
                moodLogForm.style.display = 'none';
                await loadAndRenderMoodChart();
                addMessage('assistant', `Mood logged! Score: ${score}/10 ${getMoodEmoji(score)}. Shukriya apna khayal rakhne ke liye. 💙`);
            } else if (resp.status === 403) {
                addMessage('assistant', 'Mood logging is disabled. Please enable **Mood Sync** toggle in the chat header first.');
            }
        } catch (e) {
            addMessage('assistant', 'Could not log mood. Please try again.');
        }
    });
}
...
async function loadAndRenderMoodChart() {
    try {
        const resp = await fetch(`${API_BASE}/api/session/${sessionId}`);
        if (!resp.ok) return;
        const data = await resp.json();
        const moodLog = (data.mood_log || []).slice(-7); // Last 7 entries
        renderMoodSparkline(moodLog);
    } catch (e) {
        console.warn('Could not load mood data:', e);
    }
}

function renderMoodSparkline(moodLog) {
    const svg = document.getElementById('mood-sparkline');
    if (!svg) return;

    svg.innerHTML = '';

    if (!moodLog || moodLog.length === 0) {
        svg.innerHTML = `<text x="150" y="44" text-anchor="middle" fill="rgba(255,255,255,0.4)" font-size="12">No mood entries yet. Enable Mood Sync to start tracking.</text>`;
        if (moodSummaryText) moodSummaryText.textContent = '';
        return;
    }
    ...
    const scores = moodLog.map(e => e.score);
    const n = scores.length;

    // Helper: map score (1-10) to Y coordinate
    const toY = s => padY + plotH - ((s - 1) / 9) * plotH;
    const toX = i => padX + (n === 1 ? plotW / 2 : (i / (n - 1)) * plotW);

    // --- Gradient fill ---
    const defs = document.createElementNS('http://www.w3.org/2000/svg', 'defs');
    const grad = document.createElementNS('http://www.w3.org/2000/svg', 'linearGradient');
    grad.setAttribute('id', 'moodGrad');
    ...
    // --- Filled area polygon ---
    if (n > 1) {
        const firstX = toX(0).toFixed(1), lastX = toX(n - 1).toFixed(1);
        const bottomY = (padY + plotH).toFixed(1);
        const polygon = document.createElementNS('http://www.w3.org/2000/svg', 'polygon');
        polygon.setAttribute('points', `${firstX},${bottomY} ${points} ${lastX},${bottomY}`);
        polygon.setAttribute('fill', 'url(#moodGrad)');
    ...
    // --- Score dots with emoji labels ---
    scores.forEach((s, i) => {
        const cx = toX(i), cy = toY(s);
        ...
        const dotColor = s <= 3 ? '#f87171' : s <= 5 ? '#fbbf24' : s <= 7 ? '#34d399' : '#22d3ee';
        dot.setAttribute('fill', dotColor);
        dot.setAttribute('stroke', '#0f172a');
        dot.setAttribute('stroke-width', '1.5');
        svg.appendChild(dot);

        // Emoji label on last dot
        if (i === n - 1) {
            const label = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            label.setAttribute('x', Math.min(cx + 2, W - 16));
            label.setAttribute('y', Math.max(cy - 6, padY + 8));
            label.setAttribute('font-size', '13');
            label.setAttribute('text-anchor', 'middle');
            label.textContent = getMoodEmoji(s);
            svg.appendChild(label);
        }
    });

    // --- Summary stats ---
    if (moodSummaryText && moodLog.length > 0) {
        const avg = (scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(1);
        const trend = n > 1 ? (scores[n - 1] - scores[0]) : 0;
        const trendText = trend > 0 ? `↑ trending up` : trend < 0 ? `↓ trending down` : `→ stable`;
        const lastDate = moodLog[n - 1].timestamp ? new Date(moodLog[n - 1].timestamp).toLocaleDateString('en-PK', { day: 'numeric', month: 'short' }) : 'today';
        moodSummaryText.textContent = `Last ${n} entries · Avg: ${avg}/10 ${getMoodEmoji(Math.round(avg))} · ${trendText} · Last: ${lastDate}`;
    }
}
