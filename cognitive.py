...
# ----------------------------------------------------------------------------
# 4.5 SPECIALIST RUNNER FUNCTIONS (defined outside loop to avoid closure bugs)
# ----------------------------------------------------------------------------

async def _run_psychoed_agent(normalized_text: str, history_str: str) -> tuple:
    """Runs the Psychoeducation specialist agent with RAG context and history."""
    context = await search_rag_context(normalized_text, "psychoed")
    res = await groq_client.chat.completions.create(
        model="llama-3.3-70b-versatile",
        messages=[
            {"role": "system", "content": PSYCHOED_PROMPT.format(context=context, history=history_str)},
...
async def _run_reflection_agent(normalized_text: str, history_str: str) -> tuple:
    """Runs the Reflection specialist agent with conversation history."""
    res = await groq_client.chat.completions.create(
        model="llama-3.3-70b-versatile",
        messages=[
            {"role": "system", "content": REFLECTION_PROMPT.format(history=history_str)},
            {"role": "user", "content": normalized_text}
        ],
        temperature=0.5
    )
...
async def _run_cultural_agent(normalized_text: str, history_str: str) -> tuple:
    """Runs the Cultural Framing specialist agent with RAG context and history."""
    context = await search_rag_context(normalized_text, "cultural")
    res = await groq_client.chat.completions.create(
        model="llama-3.3-70b-versatile",
        messages=[
            {"role": "system", "content": CULTURAL_PROMPT.format(context=context, history=history_str)},
            {"role": "user", "content": normalized_text}
        ],
        temperature=0.3
    )
...
async def _run_referral_agent(normalized_text: str, history_str: str) -> tuple:
    """Runs the Resource Referral specialist agent with RAG context and history."""
    context = await search_rag_context(normalized_text, "resource")
    res = await groq_client.chat.completions.create(
        model="llama-3.3-70b-versatile",
        messages=[
            {"role": "system", "content": REFERRAL_PROMPT.format(context=context, history=history_str)},
            {"role": "user", "content": normalized_text}
        ],
        temperature=0.2
    )
...
async def _run_cbt_suggestion_agent(normalized_text: str, history_str: str) -> tuple:
    """When CBT is selected but no protocol is active, gives a warm personalized exercise suggestion."""
    suggestion_prompt = (
        f"You are a compassionate CBT coach for Pakistani users. "
        f"The user has just shared this with you:\n'{normalized_text}'\n\n"
        f"Recent conversation context:\n{history_str or 'None'}\n\n"
        f"Suggest ONE specific CBT exercise from this list that would best help them right now: "
        f"Thought Record (for negative thought loops), Worry Postponement (for recurring anxious thoughts), "
        f"or Activity Scheduling (for low motivation/depression). "
        f"Explain in 2-3 warm sentences WHY this exercise will help their specific situation. "
        f"End by asking if they want to start. Keep under 80 words. "
        f"If preferred language is Urdu, use natural Roman Urdu."
    )
    try:
        res = await groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
        ...
        return "cbt", res.choices[0].message.content.strip()
    except Exception:
        return "cbt", (
            "Ek Thought Record exercise aapke liye helpful ho sakti hai — "
            "yeh negative thought loops ko analyze aur reframe karne ka structured tarika hai. "
            "Kya aap shuru karna chahenge?"
        )
...
async def run_cognitive_pipeline(session_id: str, raw_message: str, session: dict) -> tuple:
    ...
    # Build conversation history string for specialist agents (last 2 turns)
    history_str = ""
    recent_turns = session.get("turns", [])[-4:]  # Last 2 user+assistant pairs
    for turn in recent_turns:
        history_str += f"{turn['role'].capitalize()}: {turn['content'][:200]}\n"
    
    specialist_tasks = []
    for agent in selected_agents:
        # FIX: Use named standalone coroutine functions with explicit args (avoids closure bug)
        if agent == "psychoed":
            specialist_tasks.append(_run_psychoed_agent(normalized_text, history_str))
        elif agent == "reflection":
            specialist_tasks.append(_run_reflection_agent(normalized_text, history_str))
        elif agent == "cultural":
            specialist_tasks.append(_run_cultural_agent(normalized_text, history_str))
        elif agent == "referral":
            specialist_tasks.append(_run_referral_agent(normalized_text, history_str))
        elif agent == "cbt":
            specialist_tasks.append(_run_cbt_suggestion_agent(normalized_text, history_str))
